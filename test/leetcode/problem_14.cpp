...
TEST_CASE("problem_14")
{
  ...
  const std::string result_Iterator =
    leetcode::longestCommonPrefix_Iterator(input.cbegin(), input.cend());
...
TEST_CASE("problem_14")
{
  const std::vector<std::vector<int>> input = { { 1, 2, 3, 5 },
                                                { 1, 2, 3, 4 },
                                                { 1, 2, 6 } };
  const std::vector<int> expected = { 1, 2 };
  const std::vector<int> result_Iterator =
    leetcode::longestCommonPrefix_Iterator(input.cbegin(), input.cend());
  CHECK(expected == result_Iterator);
}
