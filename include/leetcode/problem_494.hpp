...
findTargetSumWays_bruteForce_Iterator(IterTy iter_begin,
                                      IterTy iter_end,
                                      const ValueTy& goal_value)
{
  if (iter_begin == iter_end) {
    ...
    return findTargetSumWays_bruteForce_Iterator(
             iter_begin + 1, iter_end, goal_value + (*iter_begin)) +
           findTargetSumWays_bruteForce_Iterator(
             iter_begin + 1, iter_end, goal_value - (*iter_begin));
  }
}
...
{
  return findTargetSumWays_bruteForce_Iterator(
    nums.cbegin(), nums.cend(), goal_value);
}
