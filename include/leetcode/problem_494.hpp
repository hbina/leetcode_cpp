...
findTargetSumWays_bruteForce_Iterator(IterTy iter_begin, IterTy iter_end,
  ...
  if (iter_begin == iter_end) {
    ...
    return findTargetSumWays_bruteForce_Iterator(iter_begin + 1, iter_end,
                                                 goal_value + (*iter_begin)) +
           findTargetSumWays_bruteForce_Iterator(iter_begin + 1, iter_end,
                                                 goal_value - (*iter_begin));
  }
}
...
static constexpr std::size_t
findTargetSumWays_bruteForce(const Iterable &nums, const ValueTy &goal_value) {
  return findTargetSumWays_bruteForce_Iterator(nums.cbegin(), nums.cend(),
                                               goal_value);
}
...
template <typename Iterable, typename ValueTy = typename std::iterator_traits<
                                 typename Iterable::iterator>::value_type>
static constexpr std::size_t
findTargetSumWays_memoization(const Iterable &nums, const ValueTy &goal_value) {
