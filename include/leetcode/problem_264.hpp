...
template <typename IntTy = std::size_t,
          typename = std::enable_if_t<std::is_integral_v<IntTy>>>
static constexpr auto nthUglyNumber(const IntTy &n) -> IntTy {
  if (n == 0)
    return 0;
  ...
  auto evaluate_tuple = [](const TupleTy &tuple) -> IntTy {
  ...
  auto find_lowest = [](const DpTy &dp) {
    return std::min_element(std::begin(dp), std::end(dp),
  ...
  DpTy dp;
  dp.emplace(std::make_pair(evaluate_tuple({0, 0, 0}), TupleTy{0, 0, 0}));
  for (DpIndexTy iter = 1; iter != static_cast<DpIndexTy>(n); iter++) {
    ...
    dp.emplace(
        std::make_pair(evaluate_tuple({a + 1, b, c}), TupleTy{a + 1, b, c}));
    dp.emplace(
        std::make_pair(evaluate_tuple({a, b + 1, c}), TupleTy{a, b + 1, c}));
    dp.emplace(
        std::make_pair(evaluate_tuple({a, b, c + 1}), TupleTy{a, b, c + 1}));
