...
static auto
checkIfExist(const std::vector<int>& arr) -> bool
{
  auto arr_c = arr;
  std::sort(std::begin(arr_c),
            std::end(arr_c),
            [](const int& lhs, const int& rhs) -> bool {
              return std::abs(lhs) < std::abs(rhs);
            });
  auto parted = std::stable_partition(
    std::begin(arr_c), std::end(arr_c), [](const int& value) -> bool {
  ...
  for (auto even_check = std::cbegin(arr_c);
       even_check != std::prev(std::cend(arr_c));
       ++even_check) {
