...
int minSetSize(const std::vector<int> &arr_input) {
  std::vector<int> arr = arr_input;
  std::sort(std::begin(arr), std::end(arr));
  auto tmp = util::generic::group_by<std::vector<int>>(
  ...
  return static_cast<int>(
      std::count_if(std::cbegin(tmp), std::cend(tmp),
                    [&](const std::vector<int> &value) -> bool {
