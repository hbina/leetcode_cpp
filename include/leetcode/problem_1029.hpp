...
template <
    ...
    typename ValueTy = typename std::iterator_traits<
        typename InnerIterTy::iterator>::value_type>
static constexpr auto twoCitySchedCost(IterTy begin, IterTy end) -> ValueTy {
  std::vector<std::array<ValueTy, 2>> transform;
  transform.reserve(std::distance(begin, end));

  std::transform(begin, end, std::back_inserter(transform),
