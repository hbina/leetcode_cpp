...
static constexpr auto get_dividing_index(const IterTy &inorder_begin,
...
static constexpr auto buildTreeTemplateInPost(const IterTy &inorder_begin,
                                              const IterTy &inorder_end,
...
static constexpr auto buildTreeInPost(const std::vector<ValueTy> &inorder,
                                      const std::vector<ValueTy> &postorder)
    -> akarithm::TreeNode<ValueTy> * {
  return buildTreeTemplateInPost(inorder.cbegin(), inorder.cend(),
