...
get_dividing_index(const IterTy& preorder_begin,
                       ...
                       const IterTy& inorder_end)
{
  ...
  root->right = buildTreeTemplatePreIn(preorder_begin + dividing_index + 1,
                                       preorder_end,
               ...
               const std::vector<ValueTy>& inorder)
{
  return buildTreeTemplatePreIn(
    preorder.cbegin(), preorder.cend(), inorder.cbegin(), inorder.cend());
}
