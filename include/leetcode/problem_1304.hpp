...
template <typename ValueTy, typename Iterable = std::vector<ValueTy>>
Iterable sumZero(const ValueTy &n) {
  using DifferenceTy = typename std::iterator_traits<
  ...
  if (static_cast<DifferenceTy>(n) % 2 == 0) {
    std::for_each(
        std::next(std::begin(result), static_cast<DifferenceTy>(n / 2)),
        std::end(result), [](ValueTy &value) { value++; });
  }
  return result;
}
