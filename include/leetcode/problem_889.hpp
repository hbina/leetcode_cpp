...
static constexpr auto constructFromPrePostTemplate(
    const IterTy &preorder_begin, const IterTy &preorder_end,
    const IterTy &postorder_begin, const IterTy &postorder_end)
  ...
  if (preorder_begin == preorder_end && postorder_begin == postorder_end)
    ...
    std::size_t size_of_partition_B =
        std::distance(postorder_begin, std::find(postorder_begin, postorder_end,
                                                 next_preorder_element)) +
        1;
    std::size_t size_of_partition_C =
        std::distance(preorder_begin, preorder_end) - 1 - size_of_partition_B;
    root->left = constructFromPrePostTemplate(
        preorder_begin + 1, preorder_begin + 1 + size_of_partition_B,
        postorder_begin, postorder_begin + size_of_partition_B);
...
constructFromPrePost(const std::vector<ValueTy> &preorder,
    ...
    -> akarithm::TreeNode<ValueTy> * {
  return constructFromPrePostTemplate(preorder.cbegin(), preorder.cend(),
